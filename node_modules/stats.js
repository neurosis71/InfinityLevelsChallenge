...
//process line to create stats
	var getLineStat = function(line, processorData){
		...
		if(line.indexOf(processorData.cpm.searchFor) != -1){
			...
			processorData.pusers.timeResponses.push(getResponseTime(line));
			...
			processorData.gusers.timeResponses.push(getResponseTime(line));
...
//process line to store response times 
	var getResponseTime = function(line){
	
		line = line.split(" ");	//split the line to be able to go trhough each information one by one
		var connect = 0, service = 0; //variables to store connect time and service time
...
//calculate the average response time
	var getAverageTime = function(times){
			...
			for(var i =0; i < times.length; i++){//we add the response times to the total
				if (typeof times[i] == "string") {//prevents string from being processed
					timeCount--;
					continue;
				} 
			...
			return total / timeCount; //we return the average response time
	}
...
//get the median reponse time
	var getMedianTime = function(times){
		...
		//remove any string in the array
...
exports.getDyno = getDyno;
exports.getAverageTime = getAverageTime;
exports.getMedianTime = getMedianTime;
